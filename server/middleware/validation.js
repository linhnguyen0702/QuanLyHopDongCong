...
const validateContract = [
  body("contractNumber")
    ...
    .withMessage("Contract number is required and must be less than 100 characters"),

  body("title").trim().isLength({ min: 5, max: 500 }).withMessage("Title must be between 5 and 500 characters"),

  body("contractorId").isInt({ min: 1 }).withMessage("Valid contractor ID is required"),

  body("value").isFloat({ min: 0 }).withMessage("Contract value must be a positive number"),
  ...
  body("status")
    .optional()
    .isIn(["draft", "pending_approval", "approved", "active", "completed", "cancelled", "expired"])
...
const validateContractor = [
  ...
  body("taxCode").optional().trim().isLength({ max: 50 }).withMessage("Tax code must be less than 50 characters"),
  ...
  body("bankName").optional().trim().isLength({ max: 255 }).withMessage("Bank name must be less than 255 characters"),
...
const validatePayment = [
  ...
  body("payment_number")
    ...
    .withMessage("Payment number is required and must be less than 50 characters"),
...
const validateNotification = [
  ...
  body("related_id").optional().isInt({ min: 1 }).withMessage("Related ID must be a positive integer"),
...
const validateApproval = [
  ...
  body("approver_id").isInt({ min: 1 }).withMessage("Valid approver ID is required"),

  body("approval_level").optional().isInt({ min: 1, max: 10 }).withMessage("Approval level must be between 1 and 10"),
