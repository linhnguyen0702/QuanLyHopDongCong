...
router.patch("/:id/read", authenticateToken, async (req, res) => {
  try {
    ...
    const [result] = await pool.execute("UPDATE notifications SET is_read = TRUE WHERE id = ? AND user_id = ?", [
      id,
      userId,
    ])
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    const { user_id, title, message, type = "info", related_table, related_id } = req.body
